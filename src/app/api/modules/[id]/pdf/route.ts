...
export async function GET(
  ...
  const tmpHtml = join(tmpdir(), `module-${id}-${Date.now()}.html`);
  const tmpPdf = join(tmpdir(), `module-${id}-${Date.now()}.pdf`);

  try {
    await writeFile(tmpHtml, html, "utf-8");

    // Use html2pdf-next.js from the pdf skill
    const pdfScript = join(
      process.cwd(),
      "skills",
      "pdf",
      "scripts",
    ...
    await new Promise<void>((resolve, reject) => {
      const child = execFile(
        "node",
        [pdfScript, tmpHtml, "--output", tmpPdf, "--nopaged"],
        { timeout: 60000 },
        (error, stdout, stderr) => {
          if (error) {
            console.error("PDF generation error:", error, stderr);
            reject(error);
          } else {
            resolve();
          }
    ...
    const filename = `${mod.title.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`;
...
/* Rich content styling (TipTap HTML) */
.rich-content {
  line-height: 1.7;
}
...
.flow-node.decision { background: #fef3c7; color: #92400e; border: 1px solid #fbbf24; transform: none; }
...
<div class="step-section">
  ...
  <div class="step-body">
    ${step.description ? `<div class="step-description"><div class="rich-content">${step.description}</div></div>` : ""}
    ...
    <div class="code-block">
      <div class="code-block-header">
        ...
        <div class="lang-label">${escapeHtml(snip.lang || "text")}</div>
      </div>
    ...
    <div class="code-block">
      <div class="code-block-header">
        ...
        <div class="lang-label">${escapeHtml(step.codeLang || "text")}</div>
      </div>
...
<div class="output-section">
  ...
  <div class="code-block">
    <div class="code-block-header">
      ...
      <div class="lang-label">${escapeHtml(mod.outputCodeLang || "text")}</div>
    </div>
